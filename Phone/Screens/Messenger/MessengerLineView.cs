...
/// elements.
/// </summary>
public partial class MessengerLineView : Control, DialogueViewBase
{
	[Signal]
	public delegate void LineCompletedEventHandler(string text, bool isRight);
	private bool firstLine = true;
	private LocalizedLine prevLine;

	[Export] private string MCName;
	[Export] private Panel textboxPanel;
	[Export] private Panel iTextboxPanel;
	[Export] private Control referenceSelf;
	...
	public override void _Ready()
	{
		...
		lineText.VisibleCharactersBehavior = TextServer.VisibleCharactersBehavior.CharsBeforeShaping;
		...
		if (continueButton == null && continueButtonPath != null && !string.IsNullOrEmpty(continueButtonPath.ToString()))
		{
	...
	private async Task RunLineInternal(LocalizedLine dialogueLine, Action onDialogueLineFinished)
	{
		async Task PresentLine()
		{
			...
			if (dialogueLine.CharacterName == MCName)
			{
				textboxPanel.Visible = true;
				iTextboxPanel.Visible = false;
				referenceSelf.SizeFlagsHorizontal = Control.SizeFlags.ShrinkEnd;
			}
			else
			{
				textboxPanel.Visible = false;
				iTextboxPanel.Visible = true;
				referenceSelf.SizeFlagsHorizontal = Control.SizeFlags.ShrinkBegin;
			}
		...
		{
			if (!String.IsNullOrEmpty(MCName) && prevLine != null)
			{
				EmitSignal(SignalName.LineCompleted, prevLine.TextWithoutCharacterName.Text, prevLine.CharacterName == MCName);
			}
		}

		currentLine = dialogueLine;
		prevLine = dialogueLine;
